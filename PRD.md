...
1. A **Chrome extension popup**: compact at-a-glance view (current net worth, daily/weekly change, 30-day sparkline).
2. A **full-page dashboard** (extension page opened in a new tab): full time-series chart, allocation breakdown, per-account detail, and data management.

Both surfaces are thin clients. All secrets, API calls, and scraping live in a **local backend server**. The browser never holds a Plaid secret or access token.

## 2. Architecture

```
chrome-extension/          (popup + dashboard, no secrets, talks only to localhost)
  popup.html / popup.js
  dashboard.html / dashboard.js
  shared/  (api client, chart components, design tokens)
server/                    (Node.js + Express, holds all secrets)
...
- **Backend:** Node.js + Express, bound to `127.0.0.1` only (never `0.0.0.0`).
- **Database:** SQLite, single file in `server/data/`, gitignored.
- **Frontend:** Chrome extension, Manifest V3. Vanilla JS or a lightweight bundled React — builder's choice, but no framework features that require remote code. Charts via Chart.js or Recharts.
...
3. **No secrets reach the browser.** The extension communicates only with `http://127.0.0.1:{PORT}`. No Plaid endpoint is called from frontend code. Code-review checklist item: grep the extension bundle for `PLAID`, `secret`, and `access-` before considering it done.
...
6. **Local attack surface minimized.** Server binds to loopback only; CORS allows only the extension's origin; basic rate limiting (e.g., `express-rate-limit`, 60 req/min) on all routes; request body size capped.
...
- **F6 — Popup:** current net worth, change vs. 7 days ago (value + %), 30-day sparkline, refresh button, and an "Open dashboard" action (`chrome.tabs.create`). Loads in under 1 second from cached data — the popup never blocks on a network call.
...
- **Popup:** same tokens at small scale (~360×480px). Number first, sparkline second, controls last.
...
1. Fresh clone → `cp .env.example .env`, fill sandbox keys → `npm install && npm run dev` → connect a Plaid sandbox bank → snapshot appears, chart renders.
2. `git log -p` and the built extension bundle contain zero secrets or tokens.
...
4. Popup renders from cache instantly with the network disabled.
...
Multi-user support, cloud deployment, transaction-level budgeting, mobile, and scraping any page other than the single configured Collectr share link.
